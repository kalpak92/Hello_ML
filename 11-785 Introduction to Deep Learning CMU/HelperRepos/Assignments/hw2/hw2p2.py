...
class VeriDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		#l_img = np.transpose(np.array(l_img.getdata()).reshape(32,32,3), (2, 0, 1))/256
		#r_img = np.transpose(np.array(r_img.getdata()).reshape(32,32,3), (2, 0, 1))/256
...
class TestVeriDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		return l_img, r_img, self.file_list[index]#.long()
...
class Test_Class_Dataset(Dataset):
	def __init__(self, X,transform=None):
		...
		for img_name in X:
			...
			#plt.show()
			#print(np.array(img.getdata()).reshape(32,32,3))
			#break
...
def train_epoch(model, train_loader, criterion, optimizer , criterion_closs, optimizer_closs):
	model.train()
...
def test_model_kaggle(model, testK_loader, criterion,idx_map):
	with torch.no_grad():
		...
		for batch_idx, (data,name) in enumerate(testK_loader):   
			...
			#print(batch_idx,name,str(5000+batch_idx) + '.jpg',predicted.cpu().numpy()[0],idx_map[predicted.cpu().numpy()[0]])
			#print(predicted)
...
def  main(args):
	...
	if not args.test:
		...
		checkpoint = torch.load('../checkpoints/0213 model_epoch_29.pth')
		...
		for i in range(start_epoch, n_epochs):
			...
			learning_rate_decay(optimizer_closs, i, lr)
			train_loss = train_epoch(model, train_loader, criterion, optimizer, criterion_closs, optimizer_closs)
			...
			torch.save({
				...
				}, '%s/0427 model_epoch_%d.pth' % ('../checkpoints', i))
			#torch.save(model.state_dict(), '%s/model_epoch_%d.pth' % ('./checkpoints', i))
		...
		if args.mode == "classification":
			...
			veri_loader_args = dict(shuffle=False, batch_size=256, num_workers=8, pin_memory=True) if cuda\
						else dict(shuffle=False, batch_size=1)
			...
			for batch_num, (l_img,r_img, labels) in enumerate(veri_loader):
				...
				#print(output.data.cpu().numpy()[0],labels.data.cpu().numpy()[0] )
				result.append([output.data.cpu().numpy()[0],labels.data.cpu().numpy()[0]])
				del l_img
				del r_img
				del labels
