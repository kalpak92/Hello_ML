    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            ...
            for l, rnn_cell in enumerate(self.rnn):
                ...
                #if l == 0:
                hidden[l] = rnn_cell(x[:,t,:],hidden[l]) if l == 0 else rnn_cell(hidden[l-1],hidden[l])
                #else:
                #    hidden[l] = rnn_cell(hidden[l-1],hidden[l])
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len-1, -1, -1):
            for l in range(self.num_layers-1, -1, -1):
                ...
                dx_t, dh[l] = self.rnn[l].backward(dh[l], self.hiddens[t+1][l], h_prev_l, self.hiddens[t][l])
                if l != 0:
                    dh[l-1] += dx_t
        dh_0 = dh / batch_size
        return dh_0
