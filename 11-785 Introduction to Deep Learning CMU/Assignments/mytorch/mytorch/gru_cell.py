...
class GRU_Cell:
    ...
    def forward(self, x, h):
        ...
        self.h_tilda = self.h_act(np.dot(self.Wh, (np.multiply(self.r, h))) + np.dot(self.Wx, x))
    ...
    def backward(self, delta):
        ...
        #print(type(self.x))
        #print(self.x.shape, np.matrix(self.x).shape)
        self.x = np.reshape(self.x, (-1,self.x.shape[0]))  
        self.hidden = np.reshape(self.hidden, (-1,self.hidden.shape[0])) 
        #print(self.x.shape, np.matrix(self.x).shape)
        ...
        self.dWzh = np.dot(d7.T, self.hidden)#np.dot(self.hidden.T, d7)
